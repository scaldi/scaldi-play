...
class ControllerInjector
  ...
  def getBindingInternal(identifiers: List[Identifier]): Option[BindingWithLifecycle] = identifiers match {
    case TypeTagIdentifier(tpe) :: Nil
        if tpe <:< typeTag[InjectedController].tpe || tpe <:< typeTag[AbstractController].tpe =>
        ...
        .find(m =>
          m.isConstructor && (m.paramLists match {
            case List(Nil, List(injectorType, paramType))
                if tpe <:< typeTag[
                  AbstractController
                ].tpe && injectorType.isImplicit && injectorType.typeSignature <:< typeTag[
                  Injector
                ].tpe && injectorType.isImplicit && paramType.typeSignature <:< typeTag[ControllerComponents].tpe =>
              true
            case List(Nil, List(injectorType))
                if tpe <:< typeTag[
                  InjectedController
                ].tpe && injectorType.isImplicit && injectorType.typeSignature <:< typeTag[Injector].tpe =>
              true
            case _ => false
          })
        )
  ...
  def getBindingsInternal(identifiers: List[Identifier]): List[BindingWithLifecycle] = getBindingInternal(
    identifiers
  ).toList
