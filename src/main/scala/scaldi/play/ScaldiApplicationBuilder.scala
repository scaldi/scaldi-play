...
final class ScaldiApplicationBuilder(
  ...
  /** Create a new Play Application using this configured builder. In order ti get the underlying injector instead,
    * please use `buildInj` or `buildPlayInj` instead.
    */
  ...
  private def copy(
      ...
      loadModules: (Environment, Configuration) => Seq[CanBeScaldiInjector] = loadModules
...
object ScaldiApplicationBuilder {
  ...
  /** Helper function that allows to construct a Play `Application` and execute a function while it's running. This
    * variation allows you to also get scaldi `Injector` of this application.
    */
  def withScaldiInj[T](
      environment: Environment = Environment.simple(),
      configuration: Configuration = Configuration.empty,
